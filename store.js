...
(function(){
  ...
  const blank = () => ({
    v: 1,
    profile: { name:'', level:'', subjects:[], language:'English', pref:'mixed', examDate:null, onboarded:false, voiceURI:null, created: Date.now() },
    topics: {}, mistakes: [], notes: [], research: [], labs: {}, projects: {},
  ...
  const Store = window.Store = {
    ...
    async connect(){
      ...
      try {
        ...
        ['profile','mistakes','notes','research'].forEach(k => { if(!remote.has(k) && this.state.meta[k]) this._dirty.add(k); });
    ...
    importJSON(text){
      ...
      const now = Date.now();
      ['profile','mistakes','notes','research'].forEach(k => this.state.meta[k] = now);
