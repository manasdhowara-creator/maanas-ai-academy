...
${src}
This lesson REPLACES a video/human-teacher explanation — it is the primary, complete way this learner masters the topic. Make it so clear, visual and well-structured that a student would prefer it over a YouTube video.
Write 6-10 sections that progress Basic → Foundation → Intermediate → Advanced → Mastery. LESS THEORY, MORE UNDERSTANDING: short paragraphs, no walls of text, no repetition or filler — but never cut content a student needs.
Each section needs:
- "body": a clear explanation (markdown, 60-160 words; short paragraphs, bullets, **bold** key terms, plain Unicode maths, definitions where relevant).
- "example": ONE concrete worked example, real-life example, or step-by-step solution.
- "visual": a diagram that makes THIS section easier to understand — pick whichever representation fits (equation steps, forces, graph, bars, labelled parts, a process/cycle, a timeline, particles, a reaction, code, an architecture, a comparison, geometry, a number line, or a keypoint) — see VISUAL spec below. Every section should have one; only omit it if truly nothing would help (rare).
- "checkpoint": ONE question (mcq/tf/fill/numerical/short) that checks THAT section only.
- "kind": "concept" for a normal teaching section, or "mistakes" for the ONE section (required) dedicated to common mistakes/misconceptions for this topic.
Preferred visual types for this subject: ${Visuals.SUBJECT_HINT[t.subjectKind] || Visuals.SUBJECT_HINT.general}
Reply ONLY JSON:
{"sections":[{"level":"Basic|Foundation|Intermediate|Advanced|Mastery","heading":"…","kind":"concept|mistakes","concepts":["c1"],"body":"markdown","example":"markdown","visual":VISUAL or null,"origin":"source|extra","sourceQuote":"exact sentence or empty","checkpoint":Q}],"summary":"3-5 sentence recap"}
...
Q format: ${Quiz.SCHEMA}`;
    ...
    secs.forEach(s => {
      ...
      const sec = { level: s.level || '', kind: s.kind === 'mistakes' ? 'mistakes' : 'concept', heading: String(s.heading || '').slice(0, 90), concepts: Array.isArray(s.concepts) ? s.concepts : [], body: String(s.body), example: String(s.example || ''), visual: (s.visual && Visuals.valid(s.visual)) ? s.visual : null, origin: t.source ? (s.origin === 'source' ? 'source' : 'extra') : 'extra', sourceQuote: String(s.sourceQuote || ''), checkpoint: cp };
...
Reply ONLY JSON {"score":0-1 (0.7+ = could genuinely teach it),"accuracy":"short","completeness":"short","reasoning":"short","missing":["…"],"misconceptions":["…"],"feedback":"2-3 sentences","followUp":"one probing question a student might ask"}`, { tier:'default', nocache:true });
  ...
  L.buildMasterNotes = (t) => L.task('notes-' + t.id, async () => {
    ...
    if(t.builtin && !AI.ready()){
      const s = Pack[t.builtin].sheet || {};
      t.masterNotes = { overview: t.overview || '', coreConcepts: s.keyConcepts || t.concepts.map(c => c.name), definitions: s.definitions || [], diagrams: [], examples: [], mistakes: s.mistakes || [], examPoints: s.tricky || [], importantQuestions: s.questions || [], memoryAids: s.hooks || [], keyFacts: s.facts || [], formulas: [], checklist: (s.keyConcepts || []).slice(0, 8) };
      saveMasterNotesText(t); Store.saveTopic(t); return;
    }
    const numeric = ['maths','physics','chemistry'].includes(t.subjectKind);
    const r = await AI.json(`Create COMPLETE, standalone "Master Notes" for this chapter/topic. A student who has NOT read the full lesson should be able to study ONLY these notes and master the chapter. Do not just summarize — compress the FULL understanding: concepts, diagrams, worked examples, mistakes, exam points. No filler, no repetition, but do not drop anything a student needs.
${AI.learner(t)}
TOPIC: ${t.title} (${t.subject}${t.level ? ', ' + t.level : ''})
CONCEPTS: ${t.concepts.map(c => c.name).join('; ')}
${t.lesson ? 'FULL LESSON (build the notes from this — every section, example and diagram idea):\n' + t.lesson.sections.map(s => `### ${s.heading}\n${s.body}\n${s.example ? 'Example: ' + s.example : ''}`).join('\n\n') : ''}
${t.source ? 'SOURCE (authoritative for source-specific facts): ' + t.source.text.slice(0, 16000) : ''}
THIS LEARNER'S OWN MISTAKES SO FAR: ${myMistakes || 'none recorded'}
${numeric ? 'This is a numerical subject: extract every relevant FORMULA the learner needs for this topic.' : 'This is not a strongly numerical subject: leave "formulas" empty and put the equivalent high-value facts in "keyFacts" instead.'}
Reply ONLY JSON:
{"overview":"2-3 sentence chapter overview",
 "coreConcepts":["4-8 core concepts, one line each"],
 "definitions":[{"term":"","meaning":""}],
 "diagrams":[1-3 VISUAL, the most important ones for this chapter],
 "examples":[{"title":"","body":"markdown worked example or worked problem"}],
 "mistakes":["common mistakes + this learner's own mistakes, up to 6"],
 "examPoints":["tricky/high-value exam points, up to 5"],
 "importantQuestions":["up to 5 likely exam questions"],
 "memoryAids":["up to 3 memory hooks/tricks, only if genuinely useful"],
 "keyFacts":["up to 6 key facts (use this for non-numeric subjects instead of formulas)"],
 "formulas": ${numeric ? '[{"formula":"e.g. v = u + at","symbols":[{"sym":"v","meaning":"final velocity"}],"unit":"e.g. m/s","whenToUse":"","example":"a worked substitution","mistake":"a common mistake with this formula"}]' : '[]'},
 "checklist":["6-10 short \\"everything you must know\\" items for final revision"]}
VISUAL = ${Visuals.SPEC_SHORT}`, { tier:'default', peek });
    const notes = {
      overview: String(r.overview || t.overview || ''),
      coreConcepts: (r.coreConcepts || []).map(String).slice(0, 10),
      definitions: (r.definitions || []).filter(d => d && d.term).map(d => ({ term: String(d.term), meaning: String(d.meaning || '') })).slice(0, 12),
      diagrams: (r.diagrams || []).filter(v => Visuals.valid(v)).slice(0, 3),
      examples: (r.examples || []).filter(e => e && e.body).map(e => ({ title: String(e.title || 'Worked example'), body: String(e.body) })).slice(0, 6),
      mistakes: (r.mistakes || []).map(String).slice(0, 8),
      examPoints: (r.examPoints || []).map(String).slice(0, 6),
      importantQuestions: (r.importantQuestions || []).map(String).slice(0, 6),
      memoryAids: (r.memoryAids || []).map(String).slice(0, 4),
      keyFacts: (r.keyFacts || []).map(String).slice(0, 8),
      formulas: numeric ? (r.formulas || []).filter(f => f && f.formula).map(f => ({ formula: String(f.formula), symbols: Array.isArray(f.symbols) ? f.symbols.map(s => ({ sym: String(s.sym || ''), meaning: String(s.meaning || '') })).slice(0, 8) : [], unit: String(f.unit || ''), whenToUse: String(f.whenToUse || ''), example: String(f.example || ''), mistake: String(f.mistake || '') })).slice(0, 10) : [],
      checklist: (r.checklist || []).map(String).slice(0, 12),
    };
    t.masterNotes = notes;
  ...
  function saveFormulasToBank(t, formulas){
    ...
    formulas.forEach(f => {
      const ex = bank.find(x => x.topicId === t.id && x.formula === f.formula);
      const entry = { id: ex ? ex.id : M.uid('fm'), topicId: t.id, topic: t.title, subject: t.subject, formula: f.formula, symbols: f.symbols, unit: f.unit, whenToUse: f.whenToUse, example: f.example, mistake: f.mistake, bookmarked: ex ? ex.bookmarked : false, created: ex ? ex.created : Date.now() };
      if(ex) Object.assign(ex, entry); else bank.unshift(entry);
    });
  ...
  function saveMasterNotesText(t){
    ...
    const n = t.masterNotes; const body = [
      n.overview ? '## Overview\n' + n.overview : '',
      n.coreConcepts.length ? '## Core concepts\n' + n.coreConcepts.map(x => '- ' + x).join('\n') : '',
      n.definitions.length ? '## Definitions\n' + n.definitions.map(d => `- **${d.term}**: ${d.meaning}`).join('\n') : '',
      n.examples.length ? '## Examples\n' + n.examples.map(e => `**${e.title}**\n${e.body}`).join('\n\n') : '',
      n.formulas.length ? '## Formulas\n' + n.formulas.map(f => `- **${f.formula}** — ${f.symbols.map(s => s.sym + ' = ' + s.meaning).join(', ')}${f.unit ? ' (unit: ' + f.unit + ')' : ''}`).join('\n') : '',
      n.keyFacts.length ? '## Key facts\n' + n.keyFacts.map(x => '- ' + x).join('\n') : '',
      n.mistakes.length ? '## Common mistakes\n' + n.mistakes.map(x => '- ' + x).join('\n') : '',
      n.examPoints.length ? '## Exam points\n' + n.examPoints.map(x => '- ' + x).join('\n') : '',
      n.importantQuestions.length ? '## Important questions\n' + n.importantQuestions.map(x => '- ' + x).join('\n') : '',
      n.memoryAids.length ? '## Memory aids\n' + n.memoryAids.map(x => '- ' + x).join('\n') : '',
      n.checklist.length ? '## Everything you must know\n' + n.checklist.map(x => '- ' + x).join('\n') : '',
    ].filter(Boolean).join('\n\n');
    if(ex){ ex.body = body; ex.updated = Date.now(); }
    else notes.unshift({ id: M.uid('n'), title: 'Master Notes: ' + t.title, body, masterNotesOf: t.id, topicId: t.id, created: Date.now(), updated: Date.now() });
    Store.touch('notes');
  }
...
VISUAL = ${Visuals.SPEC_SHORT}`, { tier:'default', peek });
  ...
  M.route('topic', (view, id) => {
    ...
    view.innerHTML = `<div class="stack">
      ...
      ${t.planned ? `<div class="chips" role="tablist">${[['study','Study'],['lesson','Lesson'],['proof','Mastery proof'],['master','Master Notes'],['sheet','One-Page Revision'],['formulas','Formula Bank'],['notes','My Notes']].map(([k, l]) => `<button class="chip ${tab === k ? 'on' : ''}" role="tab" aria-selected="${tab === k}" data-action="topic-tab" data-tid="${t.id}" data-tab="${k}">${l}</button>`).join('')}</div>` : ''}
    ...
    else if(tab === 'master') body.innerHTML = masterNotesHTML(t);
    else if(tab === 'sheet') body.innerHTML = sheetHTML(t);
    else if(tab === 'formulas') body.innerHTML = topicFormulasHTML(t);
  ...
  function doneHTML(t){
    ...
    const hasEmail = !!Store.state.profile.email;
    return stageCard('trophy', t.retainedAt ? 'Mastered and retained · 102%' : 'Mastered · 100%', pl.note,
      ...
      <div class="row" style="flex-wrap:wrap">
        <button class="btn" data-action="topic-tab" data-tid="${t.id}" data-tab="master">${M.icon.book} Master Notes</button>
        <button class="btn" data-action="topic-tab" data-tid="${t.id}" data-tab="sheet">${M.icon.note} One-Page Revision</button>
        ${hasFormulas ? `<button class="btn" data-action="topic-tab" data-tid="${t.id}" data-tab="formulas">${M.icon.target} Formula Bank</button>` : ''}
        ${hasEmail ? `<button class="btn" data-action="email-notes" data-tid="${t.id}">Email Master Notes</button>` : ''}
        ${hasEmail && hasFormulas ? `<button class="btn" data-action="email-formulas" data-tid="${t.id}">Email Formula Sheet</button>` : ''}
        ${!hasEmail ? `<a class="btn ghost" href="#settings">Add your email to enable Email Notes</a>` : ''}
        <button class="btn" data-action="start-stage" data-tid="${t.id}" data-stage="review">Revise early</button>
  ...
  function renderLesson(body, t, inStudy){
    ...
    const mode = L.readMode[t.id] || 'read';
    ...
    html += `<div class="chips"><button class="chip ${mode === 'read' ? 'on' : ''}" data-action="lesson-mode" data-tid="${t.id}" data-mode="read">${M.icon.book} Master Lesson</button><button class="chip ${mode === 'teacher' ? 'on' : ''}" data-action="lesson-mode" data-tid="${t.id}" data-mode="teacher">${M.icon.teacher} Video lesson (optional)</button></div>`;
  ...
  function readingHTML(t){
    ...
    return `<div class="stack">
      <div class="ml-nav">${nav}<a class="ml-navdot" href="#ml-sec-${t.id}-recap" title="Recap">✓</a></div>
      ${t.overview ? `<div class="card ml-hero"><div class="eyebrow">Big picture</div><div class="prose">${M.md(t.overview)}</div></div>` : ''}
      ${secs.map((s, i) => {
        const cp = s.checkpoint; const done = cp && t.lessonProgress.checkpoints[cp.id] !== undefined;
        const isMistake = s.kind === 'mistakes';
        return `<section id="ml-sec-${t.id}-${i}" class="card lesson-sec ${isMistake ? 'ml-mistake' : ''}">
          <div class="row-between"><div class="row"><span class="ml-num">${String(i + 1).padStart(2, '0')}</span><span class="level-tag">${M.esc(s.level)}</span>${isMistake ? '<span class="tag warn">' + M.icon.alert + ' Common mistake</span>' : ''}</div>${t.source ? (s.origin === 'source' ? '<span class="tag src">Source content</span>' : s.origin === 'unverified' ? '<span class="tag warn">Not found in source: extra</span>' : '<span class="tag extra">Extra explanation</span>') : ''}</div>
  ...
  function sheetHTML(t){
    ...
    return `<div class="card msheet"><div class="row-between"><div><div class="eyebrow">One-page revision</div><h2>${M.esc(t.title)}</h2></div><div class="row"><button class="btn sm" data-action="copy-sheet" data-tid="${t.id}">${M.icon.copy} Copy</button>${Store.state.profile.email ? `<button class="btn sm" data-action="email-sheet" data-tid="${t.id}">Email</button>` : ''}</div></div>
  ...
  function masterNotesHTML(t){
    ...
    if(L.inflight[k]) return `<div class="card">${M.loadingHTML('Writing your Master Notes…', 'Overview, diagrams, worked examples, mistakes, exam points — everything, compressed.')}</div>`;
    if(!t.masterNotes) return `<div class="card">${L.errors[k] ? M.errorHTML(L.errors[k].message, 'build-notes', 'Try again', `data-tid="${t.id}"`) : `<button class="btn primary" data-action="build-notes" data-tid="${t.id}">Create Master Notes</button>`}</div>`;
    const n = t.masterNotes;
    const list = (title, arr) => arr && arr.length ? `<div class="stack-sm"><h3>${M.esc(title)}</h3><ul>${arr.map(x => `<li>${M.esc(x)}</li>`).join('')}</ul></div>` : '';
    return `<div class="stack">
      <div class="card row-between"><div><div class="eyebrow">Master Notes</div><h2>${M.esc(t.title)}</h2></div>
  ...
  function formulaCardHTML(f){
    return `<div class="formula-card">
      ...
      ${f.symbols && f.symbols.length ? `<div class="fsym">${f.symbols.map(s => `<b>${M.esc(s.sym)}</b> = ${M.esc(s.meaning)}`).join(' · ')}${f.unit ? ' · unit: ' + M.esc(f.unit) : ''}</div>` : ''}
      ${f.whenToUse ? `<div class="small"><b>When to use:</b> ${M.esc(f.whenToUse)}</div>` : ''}
      ${f.example ? `<div class="small"><b>Example:</b> ${M.esc(f.example)}</div>` : ''}
  ...
  M.action('build-notes', el => { L.buildMasterNotes(T(el)); M.render(); });
  M.action('email-notes', el => Email.sendNotes(T(el), el));
  M.action('email-sheet', el => Email.sendSheet(T(el), el));
  M.action('email-formulas', el => Email.sendFormulas(T(el), el));
